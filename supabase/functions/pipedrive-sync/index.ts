...
async function syncCompanyToPipedrive(company: Record<string, unknown>) {
  // Create or update organization in Pipedrive
  const orgData = {
    name: company.name,
...
async function syncOrderToDeal(order: Record<string, unknown>, supabaseClient: any) {
  ...
  // Map order status to Pipedrive stage
  ...
  const dealData = {
    ...
    value: (order.quantity_kg as number) * 2, // Example pricing
    currency: 'EUR',
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing Pipedrive action: ${action}`);
    ...
    switch (action) {
      ...
      case 'sync_order':
        result = await syncOrderToDeal(data!, supabase);
        break;
